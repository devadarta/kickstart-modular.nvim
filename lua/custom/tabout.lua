...
--[[
  ...
  Se o próximo caractere for um delimitador de fechamento, move o cursor para fora do par.
  Caso contrário, mantém o comportamento padrão do <Tab>.
--]]
function M.smart_tab()
  -- nvim_win_get_cursor() retorna a coluna iniciando em 0.
  local row, col = unpack(vim.api.nvim_win_get_cursor(0))
  if is_closing_char(col) then
    -- Move o cursor uma posição para a direita (+1)
